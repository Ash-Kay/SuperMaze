...
#	height = floor((scr_size.y - 2 * y_margin - y_top_offset) / tile_size.y)
	y_margin += scr_size.y - height*tile_size.y
...
func backtracker(dic, start):
	...
	if SolveLine.get_point_count() < 10 and start == start_point:
		reset_endpoint()
	else:
		var solve_points = SolveLine.get_points()
		var solve_points_short = []
		SolveLine.visible = false
		if solve_points.size() > 10:
			for i in range(solve_points.size()-1, solve_points.size()-9, -1):
...
func _on_solve_pressed():
	if GameManager.save_data["hint"] > -100:
		clear_solve_line()
